...
class QuasiTile_04(QuasiTile):
    def __init__(self, oFrame0c: LocalFrame, fFaceSide: float = 20.0) -> None:
        ...
        f_tile_side = float(np.linalg.norm(as_np3(s_ref_face.vecPt2) - as_np3(s_ref_face.vecPt1)))
        ...
        for i in range(5):
            ...
            s_face_1b = IcosehedralFace(o_frame_1b, IcosehedralFace.EDef.LONG_AXIS, IcosehedralFace.EConnector.LINE, fFaceSide)
        ...
        f_max_z = f_lower_z + f_upper_z + f_tile_side
        ...
        a_middle_faces: list[IcosehedralFace] = []
        for i in range(5):
            s_face_1u = a_upper_side_faces[i]
            s_face_1l = a_lower_side_faces[4 - i]
            vec_long_1m = as_vec3(normalized(as_np3(s_face_1u.vecPt1) - as_np3(s_face_1l.vecPt1)))
            vec_short_1m = as_vec3(normalized(as_np3(s_face_1u.vecPt2) - as_np3(s_face_1l.vecPt2)))
            vec_normal_1m = as_vec3(np.cross(as_np3(vec_short_1m), as_np3(vec_long_1m)))
            o_frame_1m = LocalFrame(s_face_1l.vecPt1, vec_normal_1m, vec_long_1m)

            s_face_1m = IcosehedralFace(o_frame_1m, IcosehedralFace.EDef.LONG_AXIS, IcosehedralFace.EConnector.LINE, fFaceSide)
            a_middle_faces.append(s_face_1m)

            i_upper_index = i + 1
            if i_upper_index > 4:
                i_upper_index -= 5
            s_face_1u = a_upper_side_faces[i_upper_index]
            s_face_1l = a_lower_side_faces[4 - i]
            vec_long_1m = as_vec3(normalized(as_np3(s_face_1u.vecPt1) - as_np3(s_face_1l.vecPt1)))
            ...
            if i_lower_index < 0:
                i_lower_index += 5
            s_face_2u = a_upper_side_faces[i]
            s_face_2l = a_lower_side_faces[i_lower_index]
            vec_short_1m = as_vec3(normalized(as_np3(s_face_2u.vecPt2) - as_np3(s_face_2l.vecPt2)))
            vec_normal_1m = as_vec3(np.cross(as_np3(vec_short_1m), as_np3(vec_long_1m)))
            o_frame_1m = LocalFrame(s_face_1l.vecPt1, vec_normal_1m, vec_long_1m)
        ...
        self.m_aFaces.extend(a_middle_faces)


__all__ = ["QuasiTile_04"]
